...
/// 4-byte big-endian length-prefix protocol (matching the RustтЖФElixir wire).
...
impl WhisperSttSession {
    pub fn new(sample_rate: u32, model_name: &str) -> Result<Self> {
        ...
        let mut stdin = Box::new(worker.stdin.take()
    ...
    pub fn poll_transcript(&mut self) -> Result<Option<super::SttResult>> {
        ...
        let mut reader_taken = std::mem::replace(reader, BufReader::new(Box::new(std::io::empty()) as Box<dyn Read + Send>));
        ...
        match rx.recv_timeout(timeout) {
            ...
            Err(std::sync::mpsc::RecvTimeoutError::Timeout) => {
                tracelog::trace("stt", "STT", "Whisper worker timeout тАФ killing and re-creating on next poll");
