...
fn main() {
    ...
    let groq_key = std::env::var("GROQ_API_KEY").unwrap_or_else(|_| "NOT SET".to_string());
    let mic_device = std::env::var("TRANSLATOR_MIC_DEVICE").unwrap_or_else(|_| "NOT SET".to_string());
    
    println!("DEEPGRAM_API_KEY: {}", if deepgram_key.len() > 10 { format!("{}...", &deepgram_key[..10]) } else { deepgram_key.clone() });
    println!("GROQ_API_KEY: {}", if groq_key.len() > 10 { format!("{}...", &groq_key[..10]) } else { groq_key.clone() });
    println!("TRANSLATOR_MIC_DEVICE: {}", mic_device);
    
    let device_name = if mic_device == "NOT SET" || mic_device.is_empty() {
        "Микрофон (Jabra EVOLVE 20)"
    } else {
        &mic_device
    };
    ...
    match stream {
        Ok(s) => {
            ...
            println!("Starting stream for 5 seconds...");
            s.play().unwrap();
            std::thread::sleep(Duration::from_secs(5));
            s.pause().unwrap();
            println!("Stopped. Total samples received: {}", counter.load(Ordering::SeqCst));
        }
        Err(e) => {
